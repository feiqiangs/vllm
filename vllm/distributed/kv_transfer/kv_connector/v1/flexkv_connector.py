...
# Install: git clone https://github.com/taco-project/FlexKV.git \
...
class FlexKVConnectorV1(KVConnectorBase_V1):
    ...
    Layer loading mode:
        By default, ``wait_for_layer_load`` is enabled, which blocks per-layer
        during the forward pass to allow layer-by-layer pipelining (lower peak
        memory, higher latency).  Set ``layer_by_layer_load=False`` in
        ``kv_connector_extra_config`` to disable per-layer blocking and wait
        for all layers to finish loading after the full forward pass instead::

            --kv-transfer-config '{"kv_connector":"FlexKVConnectorV1",
              "kv_role":"kv_both",
              "kv_connector_extra_config":{"layer_by_layer_load": false}}'
    """
    ...
    def __init__(
        ...
        # Whether to block per-layer during forward pass (layer-by-layer
        # pipelining). When True, wait_for_layer_load() is called inside each
        # attention layer. When False, loading is fully async and we only wait
        # after all layers have been processed (via start_load_kv / wait_for_save).
        # Controlled by kv_connector_extra_config["layer_by_layer_load"].
        extra_config = (
            vllm_config.kv_transfer_config.kv_connector_extra_config
            if vllm_config.kv_transfer_config is not None
            else {}
        ) or {}
        self._layer_by_layer_load: bool = extra_config.get("layer_by_layer_load", True)
    ...
    def wait_for_layer_load(self, layer_name: str) -> None:
        ...
        FlexKV does not perform layer-by-layer KV loading on the worker
        side.  All transfers are coordinated by the scheduler connector
        (see :meth:`start_load_kv`), so there is nothing to wait for
        inside the attention layer.

        The ``layer_by_layer_load`` config flag and this hook are retained
        for ``KVConnectorBase_V1`` API compatibility and as a placeholder
        for a potential future layer-pipelining optimisation.

        Args:
            layer_name: the name of the layer (unused).
        """
        if not self._layer_by_layer_load:
            return
