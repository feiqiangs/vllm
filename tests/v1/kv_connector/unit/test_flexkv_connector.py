...
3. That ``wait_for_layer_load`` respects the ``layer_by_layer_load`` flag:
   when the flag is ``True`` (default) the call is forwarded to the impl;
   when ``False`` the call is a no-op.
"""
...
class TestFlexKVConnectorImportError:
    ...
    def test_import_error_message(self):
        ...
        with pytest.raises(ImportError, match="flexkv") as exc_info:
            FlexKVConnectorV1(
...
class TestWaitForLayerLoadFlag:
    """``wait_for_layer_load`` behavior should depend on ``layer_by_layer_load``."""

    def test_wait_for_layer_load_default_true(self):
        """Default (layer_by_layer_load=True): impl.wait_for_layer_load IS called."""
        impl = MagicMock()
        cfg = _make_vllm_config()  # no explicit flag → defaults to True
        connector = _build_connector(cfg, impl)

        connector.wait_for_layer_load("layer_0")
        impl.wait_for_layer_load.assert_called_once_with("layer_0")

    def test_wait_for_layer_load_explicit_true(self):
        """Explicit layer_by_layer_load=True: impl IS called."""
        impl = MagicMock()
        cfg = _make_vllm_config(layer_by_layer_load=True)
        connector = _build_connector(cfg, impl)

        connector.wait_for_layer_load("layer_1")
        impl.wait_for_layer_load.assert_called_once_with("layer_1")

    def test_wait_for_layer_load_disabled(self):
        """layer_by_layer_load=False: impl.wait_for_layer_load must NOT be called."""
        impl = MagicMock()
        cfg = _make_vllm_config(layer_by_layer_load=False)
        connector = _build_connector(cfg, impl)

        connector.wait_for_layer_load("layer_0")
        impl.wait_for_layer_load.assert_not_called()

    def test_wait_for_layer_load_disabled_multiple_layers(self):
        """Even when called for many layers, impl must not be called if disabled."""
        impl = MagicMock()
        cfg = _make_vllm_config(layer_by_layer_load=False)
        connector = _build_connector(cfg, impl)

        for i in range(32):
            connector.wait_for_layer_load(f"layer_{i}")

        impl.wait_for_layer_load.assert_not_called()

    def test_layer_by_layer_load_flag_stored(self):
        """The ``_layer_by_layer_load`` attribute reflects the config."""
        impl = MagicMock()

        cfg_default = _make_vllm_config()
        c_default = _build_connector(cfg_default, impl)
        assert c_default._layer_by_layer_load is True

        cfg_false = _make_vllm_config(layer_by_layer_load=False)
        c_false = _build_connector(cfg_false, MagicMock())
        assert c_false._layer_by_layer_load is False

        cfg_true = _make_vllm_config(layer_by_layer_load=True)
        c_true = _build_connector(cfg_true, MagicMock())
        assert c_true._layer_by_layer_load is True


class TestFlexKVConnectorExtraConfig:
    """Verify extra_config handling edge cases."""

    def test_no_kv_transfer_config(self):
        """If vllm_config has no kv_transfer_config, connector defaults to
        layer_by_layer_load=True without error."""
        impl = MagicMock()
        vllm_config = create_vllm_config(block_size=16, max_num_batched_tokens=512)
        ...
        with _install_flexkv_mock(impl):
            connector = FlexKVConnectorV1(
                vllm_config=vllm_config,
                role=KVConnectorRole.WORKER,
                kv_cache_config=_make_kv_cache_config(),
        ...
        assert connector._layer_by_layer_load is True

    def test_empty_extra_config(self):
        """Empty kv_connector_extra_config defaults to layer_by_layer_load=True."""
        impl = MagicMock()
        vllm_config = create_vllm_config(block_size=16, max_num_batched_tokens=512)
        vllm_config.kv_transfer_config = KVTransferConfig(
